...
/**
 * SQLite specific implementation of Query.
 *
 * This class reimplements methods where SQLite differs from the standard
 * implementation in Query. The only difference is the right join syntax.
 *
 * @see Query
 * @package Database
 * @version 1.4.7
...
class Sqlite extends Select
{
    ...
    /**
     * Constructs a new Sqlite object.
     *
     * @param \PDO $db
     */
    ...
    public function from()
    {
        ...
        if ( \count( $tables ) < 1 )
        {
            throw new QueryVariableParameterException( 'from', \count( $args ), 1 );
        }
     ...
     *
     * Takes 2 string arguments and returns Query.
     *
     ...
     * is returned by an QueryExpression.
     *
     * Example:
     * <code>
     * // the following code will produce the SQL
     ...
     *
     * Takes 3 string arguments and returns Query. This is a simplified form
     ...
     *
     * @throws QueryInvalidException if called with inconsistent parameters or if
    ...
    public function rightJoin()
    {
        ...
        {
            throw new QueryInvalidException( 'SELECT', "Wrong argument count passed to {$type}Join(): {$passedArgsCount}" );
        }
        ...
        if ( $passedArgsCount == 4 )
        {
            ...
            {
                throw new QueryInvalidException( 'SELECT', 'Inconsistent types of arguments passed to rightJoin().' );
            }
        }
        ...
        {
            throw new QueryInvalidException( 'SELECT', 'Invoking rightJoin() not immediately after from().' );
        }

        $table = '';
        if ( !\is_string( $args[0] ) )
        {
            throw new QueryInvalidException( 'SELECT',

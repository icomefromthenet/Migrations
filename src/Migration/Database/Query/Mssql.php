...
/**
 * SQL Server specific implementation of Query.
 *
 * This class reimplements the LIMIT method in which the
 * SQL Server differs from the standard implementation in Query.
 *
 * @see Query
 * @package Database
 * @version 1.4.7
 */
class Mssql extends Select
{
    /**
    ...
    /**
     * Same as Select::$orderString but inverted
     ...
     *        you can specify this using QuerySelect::ASC or QuerySelect::DESC
     * @return Query a pointer to $this
     */
     ...
     *
     * @throws QueryInvalidException if offset is used and orderBy is not.
     * @return string
     */
        ...
        if ( $this->hasLimit )
        {
            if ( $this->offset)
            {
                if ( !$this->orderString )
                {
                    ...
                    throw new QueryInvalidException( "LIMIT workaround for MS SQL", "orderBy() was not called before getQuery()." );
                }
                return 'SELECT * FROM ( SELECT TOP ' . $this->limit . ' * FROM ( ' . self::top( $this->offset + $this->limit, $query ) . ' ) AS DummyTable1 ' . $this->invertedOrderString . ' ) AS DummyTable2 ' . $this->orderString;
            }

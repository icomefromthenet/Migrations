...
/**
 * File containing the Select class.
 *
 * @package Database
 * @version 1.4.7
...
class Select extends Query
{
     ...
     *
     * For an introduction to aliases see {@link Query::__construct()}.
     *
     ...
     * @return Query returns a pointer to $this.
     */
     ...
     * select() will result in an QueryInvalidException.
     *
     * @throws QueryVariableParameterException if called with no parameters..
     * @throws QueryInvalidException if called after select()
     * @param string|array(string) $... Either a string with a column name or an array of column names.
     * @return Query returns a pointer to $this.
     */
    public function selectDistinct()
    {
        ...
        // Call QuerySelect::select() to do the parameter processing
        $args = \func_get_args();
     ...
     *
     * Takes the join type and two string arguments and returns Query.
     *
     ...
     *
     * Takes the join type and three string arguments and returns Query.
    ...
    protected function doJoin( $type )
    {
        ...
        if ( $passedArgsCount == 4 )
        {
            ...
            {
                throw new QueryInvalidException( 'SELECT', "Inconsistent types of arguments passed to {$type}Join()." );
            }
        }
        ...
        {
            throw new QueryInvalidException( 'SELECT', "Invoking {$type}Join() not immediately after from()." );
        }
    ...
    public function groupBy()
    {
        ...
        if ( \count( $columns ) < 1 )
        {
            throw new QueryVariableParameterException( 'groupBy', \count( $args ), 1 );
        }
    ...
    public function having()
    {
        ...
        {
            throw new QueryInvalidException( 'SELECT', 'Invoking having() not immediately after groupBy().' );
        }
        ...
        if ( \count( $expressions ) < 1 )
        {
            throw new QueryVariableParameterException( 'having', \count( $args ), 1 );
        }
    ...
    public function getQuery()
    {
        if ( $this->selectString == null )
        {
            throw new QueryInvalidException( "SELECT", "select() was not called before getQuery()." );
        }

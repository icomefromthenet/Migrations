<?php
namespace Migration\Database\Expression;

use Migration\Database\Exceptions\QueryVariableParameterException;
use Migration\Database\Query\Select;
use Migration\Database\Query\SubSelect;

/**
 * File containing the Expression class.
 *
 * @package Database
 * @version 1.4.7
...
/**
 * The Expression class is used to create database independent SQL expression.
 *
 * The Expression class is usually used through the 'expr' variable in
...
class Expression
{
     ...
     * </code>
     *
     * @throws DbAbstractionException if called with no parameters.
    ...
    public function lOr()
    {
        ...
        if ( \count( $args ) < 1 )
        {
            throw new QueryVariableParameterException( 'lOr', \count( $args ), 1 );
        }

        $elements = Select::arrayFlatten( $args );
        if ( \count( $elements ) == 1 )
        {
            return $elements[0];
        }
    ...
    public function lAnd()
    {
        ...
        if ( \count( $args ) < 1 )
        {
            throw new QueryVariableParameterException( 'lAnd', \count( $args ), 1 );
        }
    ...
    {
        $args = \func_get_args();
        $elements = Select::arrayFlatten( \array_slice( $args, 1 ) );
        $elements = $this->getIdentifiers( $elements );
        if ( \count( $elements ) < 1 )
        {
            throw new QueryVariableParameterException( $type, \count( $args ), 1 );
        }
        if ( \count( $elements ) == 1 )
        {
            return $elements[0];
     ...
     *
     * @throws QueryVariableParameterException if called with less than two
     *         parameters.
     * @throws QueryInvalidParameterException if the 2nd parameter is an
     *         empty array.
    ...
    public function in( $column )
    {
        ...
        if ( \count( $args ) < 2 )
        {
            throw new QueryVariableParameterException( 'in', \count( $args ), 2 );
        }

        if ( \is_array( $args[1] ) && \count( $args[1] ) == 0 )
        {
            throw new QueryInvalidParameterException( 'in', 2, 'an empty array', 'a non-empty array' );
        }

        $values = Select::arrayFlatten( \array_slice( $args, 1 ) );

        $column = $this->getIdentifier( $column );

        // Special handling of sub selects to avoid double braces
        if ( \count( $values ) === 1 && $values[0] instanceof SubSelect )
        {
        ...
        if ( $this->quoteValues )
        {
            foreach ( $values as $key => $value )
            {
                switch ( true )
                {
                    case $value instanceof SubSelect:
    ...
    {
        $args = \func_get_args();
        $cols = Select::arrayFlatten( $args );

        if ( \count( $cols ) < 1 )
        {
            throw new QueryVariableParameterException( 'concat', \count( $args ), 1 );
        }
     ...
     * </code>
     *
     * @throws QueryVariableParameterException
     * @return string
     */
    public function searchedCase()
    {
        ...
        if ( \count( $args ) === 0 )
        {
            throw new QueryVariableParameterException( 'searchedCase', \count( $args ), 1 );
        }

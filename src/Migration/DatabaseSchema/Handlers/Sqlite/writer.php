 ...
 * @version 1.4.4
 */
class ezcDbSchemaSqliteWriter extends ezcSqlWriter
{
    /**
     * Contains a type map from Schema types to SQLite native types.
     *
     * @var array
     */
     ...
     * @param ezcDiff $Diff
     */
    public function applyDiffToDb( ezcDbHandler $db, ezcDiff $Diff )
    {
        $db->beginTransaction();
            ...
            else
            {
                ...
                if ( strstr( $query, 'DROP COLUMN' ) ) 
                {
                    ...
                    try
                    {
                        ...
                        if ( !$matches ) 
                        {
                            throw new ezcSqliteDropFieldException( 
                    ...
                    catch ( ezcSqliteDropFieldException $e )
                    {
                    }
                    $db->commit();
                    $db->beginTransaction();
    ...
    private function dropField( $db,  $tableName , $dropFieldName )
    {
        ...
        {
            throw new ezcDropAllColumnsException( 
     ...
     * @return array(string)
     */
    public function convertDiffToDDL( ezcDiff $Diff )
    {
     ...
     *
     * @param ezcField $fieldDefinition
     * @return string
     */
    protected function convertFromGenericType( ezcField $fieldDefinition )
    {
        $typeAddition = '';
        ...
        {
            throw new ezcUnsupportedTypeException( 'SQLite', $fieldDefinition->type );
        }
     ...
     *
     * @param string           $tableName
     * @param ezcTable $tableDefinition
     */
    protected function generateCreateTableSql( $tableName, ezcTable $tableDefinition )
    {
     ...
     *
     * @param string $tableName
     * @param ezcTableDiff $tableDiff
     */
    protected function generateDiffSchemaTableAsSql( $tableName, ezcTableDiff $tableDiff )
    {
        $this->context['skip_primary'] = false;
     ...
     * @param ezcField $fieldDefinition
     */
    protected function generateAddFieldSql( $tableName, $fieldName, ezcField $fieldDefinition )
    {
     ...
     * @param ezcField $fieldDefinition
     */
    protected function generateChangeFieldSql( $tableName, $fieldName, ezcField $fieldDefinition )
    {
     ...
     * @return string
     */
    protected function generateFieldSql( $fieldName, ezcField $fieldDefinition )
    {
     ...
     * @param ezcIndex $indexDefinition
     */
    protected function generateAddIndexSql( $tableName, $indexName, ezcIndex $indexDefinition )
    {
        $sql = "";

<?php
/**
 * File containing the SchemaOracleReader class.
 *
...
use Migration\DatabaseSchema\Handlers\SqlReader;
 ...
 * @version 1.4.4
 */
class Reader extends SqlReader implements DatabaseReader
{
    /**
     * Contains a type map from Oracle native types to generic Schema types.
     *
     * @var array
     */
     ...
     * database itself and returns this schema as an Schema object.
     *
     * @return Schema
     */
    ...
    protected function fetchTableFields( $tableName )
    {
        ...
        foreach ( $resultArray as $row )
        {
            ...
            if ( in_array(
                    Helper::generateSuffixCompositeIdentName( $tableName, $row['field_pos'], 'seq' ),
                    $sequences
                 ) || in_array(
                    Helper::generateSuffixCompositeIdentName( $tableName, $row['field'], 'seq' ),
                    $sequences
                 ) )
            {
            ...
            $fieldUnsigned = false;
            $fields[$row['field']] = Schema::createNewField( $fieldType, $fieldLength, $fieldNotNull, $fieldDefault, $fieldAutoIncrement, $fieldUnsigned );
        }
    ...
    /**
     * Converts the native Oracle type in $typeString to a generic Schema type.
     *
     * This method converts a string like "VARCHAR2" to the generic Schema
    ...
    static function convertToGenericType( $typeString, &$typeLength, $typePrecision )
    {
        ...
        else
        {
            ...
            {
                throw new UnsupportedTypeException( 'Oracle', $typeString );
            }
     ...
     * Index objects. The key in the array is the index' name.
     *
     * @param  string $tableName
    ...
    protected function fetchTableIndexes( $tableName )
    {
        ...
        foreach ( $indexesArray as $row )
        {
            ...
            $indexBuffer[$keyName]['fields'][$row['column_name']] = Schema::createNewIndexField();
        ...
        {
            $indexes[$indexName] = Schema::createNewIndex( $indexInfo['fields'], $indexInfo['primary'], $indexInfo['unique'] );
        }

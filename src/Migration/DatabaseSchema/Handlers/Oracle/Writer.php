<?php
/**
 * File containing the SchemaOracleWriter class.
 *
...
{
    /**
     * Contains a type map from Schema types to Oracle native types.
     *
     * @var array
     */
    ...
    {
        $db->beginTransaction();
        foreach ( $this->convertDiffToDDL( $Diff, $db ) as $query )
        {
    ...
    */
    private function addAutoIncrementField( $db, $tableName, $autoIncrementFieldName, $autoIncrementFieldType )
    {
        ...
        $oldName = Helper::generateSuffixCompositeIdentName( $tableName, $fieldPos, "seq" );
        $oldNameTrigger = Helper::generateSuffixCompositeIdentName( $tableName, $fieldPos, "trg" );
        ...
        $newName = Helper::generateSuffixCompositeIdentName( $tableName, $fieldPos, "seq" );
        $newNameTrigger = Helper::generateSuffixCompositeIdentName( $tableName, $fieldPos, "seq" );
        // Emulation of autoincrement through adding sequence, trigger and constraint
        ...
        $constraintName = Helper::generateSuffixedIdentName( array( $tableName ), "pkey" );
     ...
     * @return array(string)
     */
    public function convertDiffToDDL( Diff $Diff, Handler $db = null )
    {
        ...
        if ( $db !== null )
        {
            $this->generateAdditionalDropSequenceStatements( $Diff, $db );
        }
     ...
     * @return void
     */
    protected function generateAdditionalDropSequenceStatements( Diff $Diff, Handler $db )
    {
        $reader = new Reader();
        $schema = $reader->loadFromDb( $db )->getSchema();
        foreach ( $Diff->removedTables as $table => $true )
        {
            foreach ( $schema[$table]->fields as $name => $field )
            {
                ...
                $seqName = Helper::generateSuffixCompositeIdentName( $table, $name, "seq" );
    ...
    protected function convertFromGenericType( Field $fieldDefinition )
    {
        ...
        {
            throw new UnsupportedTypeException( 'Oracle', $fieldDefinition->type );
        }
    ...
    protected function generateCreateTableSql( $tableName, Table $tableDefinition )
    {
        ...
        foreach ( $tableDefinition->fields as $fieldName => $fieldDefinition )
        {
            ...
            {
                $sequenceName = Helper::generateSuffixCompositeIdentName( $tableName, $fieldName, "seq" );
                $triggerName = Helper::generateSuffixCompositeIdentName( $tableName, $fieldName, "trg" );
                $constraintName = Helper::generateSuffixedIdentName( array( $tableName ), "pkey" );
